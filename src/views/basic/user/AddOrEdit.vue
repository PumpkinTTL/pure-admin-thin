<template>
  <div v-loading="showLoading" class="form-add-or-edit-container">
    <el-form ref="formRef" :rules="rules" label-width="100px" :model="form">
      ...
      <el-form-item label="用户头像">
        <div class="elegant-avatar-uploader">
          ...
          <div class="controls">
            <!-- 格式提示信息 -->
            <div class="tip">支持jpg、png、gif等格式，≤8M</div>
      ...
      <el-row :gutter="20">
        ...
        <el-col :xs="24" :sm="12">
          <el-form-item label="过期时间">
            <el-date-picker
              v-model="premiumExpireTime"
              type="datetime"
              placeholder="选择过期时间"
              format="YYYY-MM-DD HH:mm:ss"
              value-format="YYYY-MM-DD HH:mm:ss"
              :disabled="!isPremium || selectedPremiumType === 'forever'"
              style="width: 100%"
            />
          </el-form-item>
        </el-col>
      ...
      <el-row :gutter="20">
        <el-col :xs="24" :sm="12">
          <el-form-item label="自定义备注">
            <el-input
              v-model="customPremiumRemark"
              placeholder="可选填写自定义备注"
              :disabled="!isPremium"
              clearable
            />
          </el-form-item>
      ...
      <el-form-item class="form-actions">
        ...
        <!-- 添加测试数据按钮 -->
        <el-button v-if="!props.formData.id" type="info" @click="fillTestData">
          填充测试数据
        </el-button>
      </el-form-item>
    </el-form>
...
const premiumRemark = computed(() => {
  const typeLabel =
    premiumOptions.find(item => item.value === selectedPremiumType.value)
      ?.label || "会员";
  return customPremiumRemark.value
    ? `${typeLabel} - ${customPremiumRemark.value}`
    : typeLabel;
});
...
const rules = reactive<FormRules<UserForm>>({
  ...
  signature: [{ max: 100, message: "最多输入100个字符", trigger: "blur" }]
});
...
const handlePremiumTypeChange = value => {
  if (value === "forever") {
    ...
    premiumExpireTime.value =
      farFuture.toISOString().split("T")[0] + " 23:59:59";
  } else {
    // 根据选择计算过期时间
    ...
    if (option) {
      ...
      premiumExpireTime.value =
        expireDate.toISOString().split("T")[0] + " 23:59:59";
    }
  }
};
  ...
  try {
    ...
    if (avatarFile.value) {
      try {
        ...
        message("头像上传失败: " + (error.message || "请重试"), {
          type: "error"
        });
        return;
      }
    ...
    message("操作失败: " + (error.message || "请检查表单内容"), {
      type: "error"
    });
  } finally {
...
const uploadAvatar = async () => {
  ...
  try {
    ...
    // 检查上传结果
    // 200: 上传成功（新文件）
    // 201: 文件已存在，返回已有文件的URL
    // 202: 部分上传成功
    if (![200, 201, 202].includes(res.code) || !res.data || !res.data.length) {
      throw new Error(res.msg || "图片上传失败");
    }

    // 如果是重复文件，给出提示但不阻止流程
    if (res.code === 201) {
      console.log("头像文件已存在，复用已有文件:", res.data[0].url);
    }
