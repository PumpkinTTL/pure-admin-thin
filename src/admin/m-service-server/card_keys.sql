...
CREATE TABLE `bl_card_keys` (
  ...
  `valid_minutes` int(11) NOT NULL DEFAULT 0 COMMENT '有效时长(分钟)，0表示永久有效',
  ...
  KEY `idx_valid_minutes` (`valid_minutes`) COMMENT '有效时长索引，用于按有效期筛选'
...
CREATE TABLE `bl_card_key_logs` (
  ...
  `expire_time` datetime DEFAULT NULL COMMENT '过期时间，使用时根据valid_minutes计算',
...
-- 插入测试卡密数据
INSERT INTO `bl_card_keys` (`code`, `type`, `status`, `price`, `valid_minutes`, `create_time`, `use_time`, `user_id`, `remark`) VALUES
('A8SU-IJGS-82LO-DDTP', '注册邀请码', 0, NULL, 0, NOW(), NULL, NULL, '测试用永久有效注册码'),
('B9TV-JKHT-93MP-EEUQ', '商品兑换码', 0, 99.00, 43200, NOW(), NULL, NULL, '测试用30天有效商品兑换码'),
('C0UW-KLIU-04NQ-FFVR', 'VIP兑换码', 1, 199.00, 10080, NOW(), NOW(), 1, '测试用7天VIP码-已使用'),
('D1VX-LMJV-15OR-GGWS', '积分兑换码', 2, NULL, 1440, NOW(), NULL, NULL, '测试用1天积分码-已禁用'),
('E2WY-MNKW-26PS-HHXT', '注册邀请码', 0, NULL, 0, NOW(), NULL, NULL, '测试用永久有效注册码2');
...
SELECT 
  ...
  END AS status_text,
  CASE 
    WHEN ck.valid_minutes = 0 THEN '永久有效'
    WHEN ck.valid_minutes < 60 THEN CONCAT(ck.valid_minutes, '分钟')
    WHEN ck.valid_minutes < 1440 THEN CONCAT(FLOOR(ck.valid_minutes / 60), '小时')
    ELSE CONCAT(FLOOR(ck.valid_minutes / 1440), '天')
  END AS valid_text,
  CASE 
    WHEN ck.status != 1 THEN 0
    WHEN ck.valid_minutes = 0 THEN 0
    WHEN TIMESTAMPDIFF(SECOND, ck.use_time, NOW()) > (ck.valid_minutes * 60) THEN 1
    ELSE 0
  END AS is_expired
...
BEGIN
  ...
  WHILE i < p_count DO
    ...
    -- 插入卡密
    INSERT INTO bl_card_keys (code, type, status, price, valid_minutes, create_time, remark)
    VALUES (v_code, p_type, 0, p_price, p_valid_minutes, NOW(), p_remark);
...
1. bl_card_keys - 卡密主表
   - 使用唯一索引确保卡密码不重复
   - status字段控制卡密状态
   - valid_minutes为0表示永久有效
   - price字段可为NULL，适配不需要价格的卡密类型

2. bl_card_key_logs - 使用记录表
   - 记录所有卡密操作历史
   - expire_time在使用时计算: use_time + valid_minutes
   - 支持追溯完整的卡密生命周期
